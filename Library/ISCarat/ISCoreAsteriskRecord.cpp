...
bool ISCoreAsteriskRecord::Invoke()
{
	...
	{
		ISLOGGER_W("Directory with voice records not setting");
		return false;
	}

	if (!QDir(RepositoryPath).exists()) //Если папка с хранилищем записей разговоров не существует
	{
		ISLOGGER_W("Directory with voice record not exist: " + RepositoryPath);
		return false;
	}
	...
	{
		ISLOGGER_W("Invalid port: " + QString::number(ListenPort));
		return false;
	}
	...
	if (!Extension.length())
	{
		ISLOGGER_W("Extension for voice record not setting");
		return false;
	}
	...
	{
		ISLOGGER_I("Listen port: " + QString::number(TcpServer->serverPort()));
		connect(TcpServer, &QTcpServer::newConnection, this, &ISCoreAsteriskRecord::NewConnection);
		Started();
	}
	else
	{
		ISLOGGER_I("Error listen port: " + QString::number(TcpServer->serverPort()));
		return false;
	}
...
void ISCoreAsteriskRecord::NewConnection()
{
	...
	if (TcpSocket)
	{
		ISLOGGER_I("New connection client from: " + ISNetwork().ParseIPAddress(TcpSocket->peerAddress().toString()));
...
void ISCoreAsteriskRecord::Disconnected()
{
	...
	{
		TcpSocket->close();
		ISLOGGER_I("Disconnected client from: " + ISNetwork().ParseIPAddress(TcpSocket->peerAddress().toString()));
	}
}
//-----------------------------------------------------------------------------
void ISCoreAsteriskRecord::ReadyRead()
{
	...
	ISLOGGER_I("Search file: " + FileRecord.fileName());
	if (!FileRecord.exists()) //Запрашиваемый файл не существует
	{
		ISLOGGER_W("File \"" + FileName + "\" not exist");
	...
	{
		ISLOGGER_W("Error open file \"" + FileName + "\": " + FileRecord.errorString());
	...
	while (!Vector.empty()) //Отправка файла
	{
		...
		if (TcpSocket->isOpen()) //Если сокет открыт
		{
			...
			ISLOGGER_I("Sended " + QString::number(Sended / 1000) + "kb of " + QString::number(Size / 1000) + "kb");
			ISSystem::ExecLoop(10); //Задержка в 50 msec
		}
		else //Сокет закрылся
		{
			ISLOGGER_I("Stop send file");
			return;
		}
	}

	ISLOGGER_I("Sending finish");
