...
void ISCoreTelephony::HandlingCDR(int ID)
{
	...
	if (SRC.length() == AsteriskDigitNumbers && DST.length() == AsteriskDigitNumbers) //Внутренний вызов
	{
		...
		Subscriber = SRC;
		Number = DST;
		ISLOGGER_I("Handling internal call from \"" + Subscriber + "\" to number \"" + Number + "\". Call id: " + QString::number(ID));
...
QVariant ISCoreTelephony::GetDialStatus(int ID, const QString &DialStatus) const
{
	...
	{
		ISLOGGER_W("Not found dial status \"" + DialStatus + "\". Call id: " + QString::number(ID));
	}
...
QString ISCoreTelephony::GetPattern(int ID, const QString &String) const
{
	...
	{
		ISLOGGER_W("Error parse patter from string: " + String + ". Call id: " + QString::number(ID));
	}
...
QVariant ISCoreTelephony::GetUser(int ID, const QVariant &Pattern) const
{
	...
	if (!UserID.isValid())
	{
		ISLOGGER_W("Not found user with pattern \"" + Pattern.toString() + "\". Call id: " + ID);
	}
	return UserID;
}
//-----------------------------------------------------------------------------
