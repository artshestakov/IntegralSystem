...
bool ISMetaSystemsEntity::Initialize()
{
	...
	if (Result)
	{
		while (qSelect.Next())
		{
			...
			if (!ISBuffer::Instance().CurrentUserInfo.System) //Если текущий пользователь не системный
			{
				if (!ISBuffer::Instance().CurrentUserInfo.GroupFullAccess) //Если у группы пользователя нет полного доступа - проверять доступ к подсистемам
				{
