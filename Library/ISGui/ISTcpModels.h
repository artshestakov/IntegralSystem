...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
struct ISModelLogRecord
{
	QString Color;
	QStringList Values;
};
//-----------------------------------------------------------------------------
class ISLogModel : public QAbstractItemModel
{
	Q_OBJECT

public:
	ISLogModel(QObject *parent);
	virtual ~ISLogModel();
...
private:
	unsigned int GetCount(const QString &Severity) const; //Получить количество сообщений по типу

private:
	std::vector<QString> Fields;
	std::vector<ISModelLogRecord> Records;
};
//-----------------------------------------------------------------------------
#endif
