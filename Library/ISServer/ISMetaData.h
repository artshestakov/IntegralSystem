...
public:
    ...
    const std::vector<ISResourceItem>& GetVectorXSN() const;
    bool CheckExistResource(const std::string &UID) const;

private:
    bool XSNInit();
    bool XSNInit(const unsigned char *Content, size_t Size, const std::string &FileName, tinyxml2::XMLElement *XmlElementTemplateXNS);
    ...
    bool XSRInit();
    bool XSRInit(const unsigned char *Content, size_t Size, const std::string &FileName);
    bool XSRInit(tinyxml2::XMLElement *XmlElement);

    bool XSFInit();
    bool XSFInit(const unsigned char *Content, size_t Size, const std::string &FileName);
...
private:
    ...
    std::vector<ISResourceItem> VectorXSN, VectorXSR, VectorXSF;
