...
std::string ISAlgorithm::FormatNumber(int64_t Number, char Separator)
{
    ...
    {
        int Size = (int)strlen(Char); //Рассчитаем размер строки
...
std::string ISAlgorithm::FormatNumber(double Number, char Separator, unsigned int Precision)
{
    ...
    //Тесты показали что так работает быстрее.
    if (Precision == 0)
    {
        return FormatNumber(ValueLeft, Separator);
    }

    int Factor = 1; //Множитель
    ...
    if (Value < 0) //Правая часть дроби не может быть отрицательной
    {
        Value = -Value;
    }
