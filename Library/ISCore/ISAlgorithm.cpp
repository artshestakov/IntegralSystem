...
//-----------------------------------------------------------------------------
std::vector<std::string> ISAlgorithm::ParseCommandArgs(int argc, char **argv)
{
...
//-----------------------------------------------------------------------------
std::string ISAlgorithm::FromLocal8Bit(const char *String)
{
	int ResultU = MultiByteToWideChar(CP_UTF8, 0, String, -1, 0, 0);
	if (!ResultU)
	{
		return std::string();
	}

	wchar_t *WChar = new wchar_t[ResultU];
	if (!MultiByteToWideChar(CP_UTF8, 0, String, -1, WChar, ResultU))
	{
		delete[] WChar;
		return 0;
	}

	int ResultC = WideCharToMultiByte(1251, 0, WChar, -1, 0, 0, 0, 0);
	if (!ResultC)
	{
		delete[] WChar;
		return 0;
	...
	char *Char = new char[ResultC];
	if (!WideCharToMultiByte(1251, 0, WChar, -1, Char, ResultC, 0, 0))
	{
		delete[] Char;
		return 0;
	}
